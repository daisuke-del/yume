...
$name2 = preg_replace("/(| )/", "",$name);
      ...
      try {
        ...
        //SQL文を作る
        $sql = "SELECT name,image,instagram,twitter,video FROM bbs2 WHERE kana LIKE(:name2)";
        //プリペイドステートメントを作る
        $stm = $pdo->prepare($sql);
        //プレースホルダに値をバインドする
  ...
  <body>
    <div>
      ...
      <?php foreach ($stm as $row): ?>
        ...
        <?php echo $row[0]?><br><a class="thumbnail" href="<?php echo $row[1]?>" target="_blank"><image src="<?php echo $row[1]?>"></a><br>
